...
const validateName = (name, fieldLabel) => {
  ...
  if (/[-']{2,}/.test(trimmedName)) {
    return {
      isValid: false,
      message: 'No consecutive hyphens or apostrophes allowed',
    };
  }
  ...
  if (/-'|'-/.test(trimmedName)) {
    return {
      isValid: false,
      message: 'Hyphen and apostrophe cannot be adjacent',
    };
  }
  ...
  if (/^[-']|[-']$/.test(trimmedName)) {
    return {
      isValid: false,
      message: 'Name cannot start or end with hyphen or apostrophe',
    };
  }
...
export const validatePhoneNumber = (number, countryCode) => {
  ...
  try {
    ...
    if (!isValidPhoneNumber(fullNumber)) {
      return {
        isValid: false,
        message: `Invalid phone number format for ${country.country}. Example: ${country.example}`,
      };
    }
    ...
    if (!phoneNumber.isValid() || phoneNumber.country !== country.countryCode) {
      return {
        isValid: false,
        message: `Invalid phone number for ${country.country}. Example: ${country.example}`,
      };
    }
    ...
    if (type && !VALID_PHONE_TYPES.includes(type)) {
      return {
        isValid: false,
        message: `Please enter a valid mobile or landline number. Example: ${country.example}`,
      };
    }
